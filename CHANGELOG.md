...
- External anchoring, two ways: (1) publish the head by hand and re-check with `skillwatch verify --against <head>` (zero dependencies); or (2) `skillwatch anchor` obtains a signed RFC 3161 timestamp for the head from a public authority (freeTSA.org by default) under the optional `[anchor]` extra. Anchors are stored in a new `anchors` table, and `skillwatch verify` now automatically checks every recorded anchor — confirming the anchored head is still in the chain and cryptographically verifying the token — so a full-chain rewrite that plain `verify` would accept is caught. Only a hash ever leaves the machine; the core stays offline (freeTSA's CA cert is bundled for offline verification).

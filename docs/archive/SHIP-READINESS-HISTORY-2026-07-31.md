# Historical ship-readiness narrative — archived 2026-07-31

This file preserves the former `SHIP-READINESS.md` verbatim below this notice.
It is historical evidence, not current status and not an input to the current
verdict. Several statements conflict with the current harness or with each other;
that contradiction is why it was archived rather than retained as a scoreboard.
Current authority: `docs/readiness-status.json` and `SHIP-READINESS.md`.

# Former Ship Readiness
